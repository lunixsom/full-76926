...
import { deleteImageFile, imageFileExists } from "../utils/imageFileHandler.js";
...
router.get("/:id", async (req, res) => {
    try {
        ...
        if (!shoe) {
            return res.status(404).json({ status: "error", message: "El id no corresponde a ningun zapato registrado" });
        }
...
router.put("/:id", uploader.single("thumbnail"), async (req, res) => {
    try {
        ...
        if (req.file && req.file?.filename !== shoe.thumbnail) {
            if (shoe.thumbnail && shoe.thumbnail !== "default.jpg" && await imageFileExists(shoe.thumbnail)) {
